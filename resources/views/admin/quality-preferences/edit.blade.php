        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ trans('messages.quality_preferences_edit') }}</h5>
                </div>

                <div class="card-body">
                    <form method="POST" data-parsley-validate="" id="addEditForm" role="form">
                        @csrf
                        <input type="hidden" id="edit_value" value="{{ $quality_preferences->id }}" name="edit_value">
                        <div class="row">
                            <div class="col-lg-12 mb-3">
                                <label for="panel_brand_id"
                                    class="form-label required">{{ trans('messages.panel_brand_id') }}</label>
                                <select class="form-select" data-choices name="panel_brand_id" id="panel_brand_id"
                                    ...
                                    @if (!is_null($panel_brandss))
                                        @foreach ($panel_brandss as $panel_brands)
                                            <option value="{{ $panel_brands->id }}"
                                                {{ $quality_preferences->panel_brand_id == $panel_brands->id ? 'selected' : '' }}>
                                                {{ $panel_brands->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="col-lg-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="name" name="name"
                                    value="{{ $quality_preferences->name }}"
                            ...
                            <div class="col-lg-12 mb-3">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="4"
                                    placeholder="{{ trans('messages.description') }}">{{ $quality_preferences->description }}</textarea>
                            </div>
                            <div class="text-end">
